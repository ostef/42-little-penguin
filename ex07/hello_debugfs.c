...
static ssize_t jiffies_read(struct file *f, char __user *buff, size_t len, loff_t *offset)
{
        ...
        int written = snprintf(jiffies_buffer, sizeof(jiffies_buffer), "%lu", get_jiffies_64());
...
static int __init hellodebugfs_init(void)
{
        ...
        if (!jiffies_file) {
                pr_error("Could not create 'jiffies' file\n");
                debugfs_remove_recursive(debug_dir);
        ...
        if (!foo_file) {
                pr_error("Could not create 'foo' file\n");
                debugfs_remove_recursive(debug_dir);
